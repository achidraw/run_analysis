...
run_analysis <- function(){
        ...
        colnames(std_mean_data)[1:2] <- c("subject", "activity")

        ## merge the two tables - activity and std_mean_data - since merge adds the y related columns to the end, we pass activity as x and std_mean_data as y to merge() function
        ## also since we want all the rows to be present in new data frame (activity is more a lookup of the activity id's and labels), we pass all.y=TRUE
        mergedata1 <- merge(activity, std_mean_data, by.x="activity", by.y="activity",all=FALSE, all.y=TRUE)
        
        ## aggregate() the data based on grouping of subject, activity_label and activity. these grouping variables need to be fators in a list - - subject and activity are int and hence need to be converted to factor
        ...
        tidydataset <- aggregate(mergedata1, by=list(factor(mergedata1$subject), factor(mergedata1$activity), mergedata1[[2]]), FUN=mean, na.rm=TRUE)

        ## the dataframe created by aggregate now has three group columns one each for the grouping by subject, activity and activity_label. And it also tries to do the mean of those columns along with the other 561 measurement columns
        ## hence we remove the old 3 columns and rename the group columns
        finaltidydataset <- tidydataset[,-(4:6)]
        colnames(finaltidydataset)[1:3] <- c("subject", "activity", "activity_label")
